...
int insertSymbolTable(enum symbolType kind, char name[]) {
    ...
    switch (kind) {
        case function : {   //函数
            for(int i=0; i<symbolTable.size(); i++) { // 检查函数符号表
...
int term(tree* father) {    //< term >::=<factor>{(*| /)< factor >}
    ...
    while(strcmp("*", type)==0 || strcmp("/", type)==0||strcmp("%", type)==0) {
        ...
        if(strcmp("*", temp) == 0)
            ...
            strcpy(codes[codeLength++].operationCode, "MOD") ;       //除
    }
    return flag ;
}
...
int declaration_list(tree* father) {    
    ...
    while(1) {
        ...
        //不是int说明已经没有变量声明了进入语句内容
            break ;
        flag = declaration_stat(current) ;     
        //具体的声明内容
        if(flag > 0)
            return flag ;
    }
...
int function_body(tree* father) {       
    ...
    //每次进入一个函数体的时候，相对地址都要重置为2
    symbolTable[symbolLength-1].address = codeLength ;

    //函数体的入口地址，上一个存在符号表中的是函数名
    flag = declaration_list(current) ; 

    //如果成功了当前的单词是语句的内容
    if(flag > 0)
        return flag ;
    strcpy(codes[codeLength].operationCode, "ENTER") ;  
    //为函数调用开辟空间
    ...
    flag = statement_list(current) ; 
     ...
     //一开始不用读取新的内容，成功了当前的单词应该是 }
    if(strcmp("}", type) == 0){
        getNext();
    }
    if(flag > 0)
        return flag ;
    strcpy(codes[codeLength].operationCode, "RETURN") ;  
    //函数返回
    codeLength++ ;
    add(current) ;
    //getNext() ;
...
int main_declaration(tree* father) { 
    ...
    //程序中最后的声明必须是名字为 main 的主函数
        return 6 ;
    add(current) ;

    flag = insertSymbolTable(function, value) ;    
    //将函数名添加到符号表中
...
int program() {     //<program> ::={fun_declaration }<main_declaration>
    ...
    strcpy(codes[codeLength].operationCode, "BR") ;    
    //程序开始先跳转到main函数
    // BR 无条件转移
